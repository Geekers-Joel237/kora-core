...
import com.geekersjoel237.koracore.payment.config.DataInitializer;
...
class FinancialInvariantsDbTest extends AbstractRepositoryTest {
    ...
    @Autowired private CashInCommandHandler cashInCommandHandler;
    @Autowired private CashOutCommandHandler cashOutCommandHandler;
    ...
    private SetupResult setup(String email, String phoneNumber) {
        ...
        Customer customer = Customer.create(user, PhoneNumber.of("+225", phoneNumber), Pin.of("1234"), pinEncoder);
    ...
    void double_entry_is_maintained_after_cash_in() {
        ...
        cashInCommandHandler.execute(new CashInCommand(Id.generate(), ctx.customerId(), Pin.of("1234"), CASH_IN_AMOUNT, PaymentMethod.ORANGE_MONEY));
        em.flush();
    ...
    void each_cash_in_transaction_has_exactly_two_operations() {
        ...
        PaymentResult result = cashInCommandHandler.execute(
                new CashInCommand(Id.generate(), ctx.customerId(), Pin.of("1234"), CASH_IN_AMOUNT, PaymentMethod.ORANGE_MONEY));
        em.flush();
    ...
    void double_entry_is_maintained_per_transaction_after_cash_in_and_cash_out() {
        ...
        cashInCommandHandler.execute(
                new CashInCommand(Id.generate(), ctx.customerId(), Pin.of("1234"), CASH_IN_AMOUNT, PaymentMethod.ORANGE_MONEY));
        cashOutCommandHandler.execute(
                new CashOutCommand(Id.generate(), ctx.customerId(), Pin.of("1234"), CASH_OUT_AMOUNT, PaymentMethod.ORANGE_MONEY));
        em.flush();
