package com.geekersjoel237.koracore.payment.e2e;

import com.geekersjoel237.koracore.shared.e2e.AbstractE2ETest;
import com.geekersjoel237.koracore.payment.adapters.in.rest.api.cashIn.CashInRequest;
import com.geekersjoel237.koracore.payment.adapters.in.rest.api.cashOut.CashOutRequest;
import com.geekersjoel237.koracore.payment.adapters.in.rest.api.history.TransactionHistoryResponse;
import com.geekersjoel237.koracore.payment.adapters.in.rest.api.transfer.TransferRequest;
...
class TransactionHistoryE2ETest extends AbstractE2ETest {
    ...
    // ── Test 7: the paging defaults and bounds ────────────────────────────────

    @Test
    void should_apply_the_default_page_when_the_client_asks_for_none() {
        cashIn(alice, 1_000);

        ResponseEntity<TransactionHistoryResponse> response = getHistory(alice, null);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        Assertions.assertNotNull(response.getBody());
        assertThat(response.getBody().page()).isZero();
        assertThat(response.getBody().size()).isEqualTo(Pagination.DEFAULT_SIZE);
    }

    @Test
    void should_apply_the_default_size_when_only_the_page_is_given() {
        cashIn(alice, 1_000);

        ResponseEntity<TransactionHistoryResponse> response = getHistory(alice, "page=0");
    ...
    /**
     * A negative page reaches the SQL as a negative OFFSET, which Postgres refuses. The
     * bound is checked before the query is built, so the client gets a 400, not a 500.
     */
    @Test
    void should_reject_a_negative_page() {
        ResponseEntity<String> response = getWithToken("/payments/history?page=-1",
                alice.tokens().accessToken(), String.class);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
    }

    @Test
    void should_reject_a_page_over_the_ceiling() {
        ResponseEntity<String> response = getWithToken(
                "/payments/history?size=" + (Pagination.MAX_SIZE + 1),
                alice.tokens().accessToken(), String.class);
    ...
    @Test
    void should_reject_an_empty_page() {
        ResponseEntity<String> response = getWithToken("/payments/history?size=0",
                alice.tokens().accessToken(), String.class);
