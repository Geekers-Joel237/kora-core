...
import com.geekersjoel237.koracore.auth.adapters.in.rest.api.login.LoginRequest;
import com.geekersjoel237.koracore.auth.adapters.in.rest.api.refreshToken.RefreshRequest;
...
public abstract class AbstractE2ETest {
    ...
    @Autowired
    protected ExpiringStore<OtpCode> otpCodes;
    ...
    protected String waitAndGetOtpCode(String email) {
        ...
        for (int i = 0; i < 20; i++) { // up to ~1s total
            var otpOpt = otpCodes.get(key);
            if (otpOpt.isPresent()) {
                return otpOpt.get().value();
            }
            try {
                Thread.sleep(50);
